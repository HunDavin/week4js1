...
const Update_Data =[
  {lastName:"Jing",batch:"A1"},
  {lastName:"Minh",batch:"A2"},
  {lastName:"Puch",batch:"A3"},
  {lastName:"Jung",batch:"A4"},
];


const FINAL_DATA = STUDENTS_DATA.map((student,index)=>({...student,...Update_Data[index]}));
...
function updateStudentAge(firstName,lastName,batch, newAge) {
  let student = FINAL_DATA.find((s) => s.firstName === firstName&&s.lastName===lastName&&s.batch===batch);
  if (student) {
    student.age = newAge;
  }
}
...
updateStudentAge("An","Jing","A1", 30);
